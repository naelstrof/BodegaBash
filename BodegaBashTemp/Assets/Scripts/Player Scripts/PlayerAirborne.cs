...
public class PlayerAirborne : PlayerState {
	...
	public override void Update(PlayerController player)
	{
		...
		if (vert + horz != 0) {
			...
			player.body.transform.rotation = Quaternion.RotateTowards (player.body.transform.rotation, desiredRotation, 360*Time.deltaTime*temp.magnitude);
			//player.body.transform.LookAt (player.body.transform.position + temp);
		} else {
			...
			player.body.transform.rotation = Quaternion.RotateTowards (player.body.transform.rotation, desiredRotation, 360*Time.deltaTime);
